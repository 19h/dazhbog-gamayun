<p align="center">
  ...
  <em>A Binary Ninja Lumina client for dazhbog with IDA-parity function hashing, metadata decoding, and safe application workflows.</em>
</p>

<p align="center">
  <code>C++20</code> &middot; <code>Binary Ninja UI</code> &middot; <code>Qt6</code> &middot; <code>Lumina v6</code> &middot; <code>hash parity</code> &middot; <code>read-only client</code>
</p>
...
| Area | What it does |
|------|---------------|
| **Function-hash engine** | Computes IDA-style `MD5(normalized_bytes || mask_bytes)` signatures from Binary Ninja analysis |
...
| **Debug tooling** | Emits detailed function-hash dumps and ships comparison/verification scripts for parity work against IDA |
...
- getting function hashing close enough to IDA that lookups are actually useful
...
- **Computes Lumina-compatible function keys** using a dedicated function-hash masking engine in `src/analysis/`
...
Clicking a row also computes and logs that function's hash for quick triage.
...
Binary Ninja analysis
        |
        v
Function-hash engine (src/analysis/)
        |
        +--> reliability filter
        |
        v
...
2. Gamayun computes function hashes for selected or eligible functions.
...
- `src/analysis/` - masking and function-hash generation
...
| `LUMINA_DEBUG` | Enable extra function-hash and pull debug behavior |
...
For AArch64 instruction-mask comparisons, `--ida-split-size 4` matters because IDA can emit multi-instruction macro records spanning multiple 4-byte instructions.
...
- **Best-effort parity, not perfect identity** - the function-hash engine is strong, but Binary Ninja / IDA function-layout disagreements can still exist, especially around split tails
...
It gives you a native UI, a parity-minded function-hash engine, structured metadata decoding, safe application workflows, and the tooling needed to keep improving the match quality over time.
